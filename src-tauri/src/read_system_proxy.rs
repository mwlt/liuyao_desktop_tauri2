...
mod linux_proxy {
        ...
        match run_command("gsettings", &["get", "org.gnome.system.proxy", "mode"]) {
            Ok(mode) => {
                ...
                if proxy_info.proxy_enabled {
                    // 获取 HTTP 代理
                    if let Ok(host) =
                        run_command("gsettings", &["get", "org.gnome.system.proxy.http", "host"])
                    {
                        if let Ok(port) = run_command(
                            "gsettings",
                            &["get", "org.gnome.system.proxy.http", "port"],
                        ) {
                            proxy_info.http_proxy =
                                format!("{}:{}", host.trim_matches('\''), port.trim());
                        }
                    }

                    // 获取 HTTPS 代理
                    if let Ok(host) = run_command(
                        "gsettings",
                    ...
                    // 获取不代理列表
                    if let Ok(no_proxy) = run_command(
                        "gsettings",
    ...
    pub fn get_xfce_proxy() -> Option<SystemProxyInfo> {
        ...
        match run_command(
            "xfconf-query",
            &["-c", "xfce4-settings", "-p", "/system/proxy/mode"],
        ) {
            Ok(mode) => {
                ...
                if proxy_info.proxy_enabled {
                    ...
                    if let Ok(host) = run_command(
                        "xfconf-query",
                        &["-c", "xfce4-settings", "-p", "/system/proxy/http/host"],
                    ) {
                        if !host.trim().is_empty() {
                            if let Ok(port) = run_command(
                                "xfconf-query",
                                &["-c", "xfce4-settings", "-p", "/system/proxy/http/port"],
                            ) {
                    ...
                    if let Ok(host) = run_command(
                        "xfconf-query",
                        &["-c", "xfce4-settings", "-p", "/system/proxy/https/host"],
                    ) {
                        if !host.trim().is_empty() {
                            if let Ok(port) = run_command(
                                "xfconf-query",
                                &["-c", "xfce4-settings", "-p", "/system/proxy/https/port"],
                            ) {
                                if !port.trim().is_empty() {
                                    proxy_info.https_proxy =
                                        format!("{}:{}", host.trim(), port.trim());
                    ...
                    if let Ok(host) = run_command(
                        "xfconf-query",
                        &["-c", "xfce4-settings", "-p", "/system/proxy/socks/host"],
                    ) {
                        if !host.trim().is_empty() {
                            if let Ok(port) = run_command(
                                "xfconf-query",
                                &["-c", "xfce4-settings", "-p", "/system/proxy/socks/port"],
                            ) {
                                if !port.trim().is_empty() {
                                    proxy_info.socks_proxy =
                                        format!("{}:{}", host.trim(), port.trim());
                    ...
                    if let Ok(no_proxy) = run_command(
                        "xfconf-query",
                        &["-c", "xfce4-settings", "-p", "/system/proxy/no_proxy"],
                    ) {
                        proxy_info.no_proxy = no_proxy.trim().to_string();
                    }
